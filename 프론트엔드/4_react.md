...
### 불변성
데이터를 한 번 생성하면 바꿀 수 없음 

=> 데이터의 변화를 감지하여 UI를 업데이트하는데, 불변성을 유지해야 데이터의 변화가 효율적으로 감지됨

=> 가상 DOM이 이전 상태와 현재 상태를 비교할 때, 객체의 참조가 변경되었는지만 확인하면 되기 때문 

1. 상태 변경 예측 -> 디버깅을 용이하게 하며, 코드의 안정성을 높임

2. 성능 최적화 -> 리액트는 props/state 변경 감지에 얕은 복사를 사용하는데 불변성을 유지하면 불필요한 렌더링을 방지할 수 있음
...
-   변경이전 값으로 clean up 함수 호출되고 변경된 값으로 setup 함수 실행
...
리액트는 이벤트 위임을 사용해 실제 돔 요소가 아닌 루트에서 모든 이벤트를 관리 
...
리액트 컴포넌트를 순순하게 유지해야 함. (이때 이벤트 핸들러는 사이드 이펙트)

1. 많은 사용자 입력을 처리하기 위해서

2. 순수 함수는 캐시하기 안전함, 입력이 바뀌지 않으면 랜더링을 건너뛸 수 있음  

3. 연산이 중단되어도 안전함, 깊은 컴포넌트 트리를 렌더링하는 도중에 중단되어 다시 시작해도 괜찮음 
...
Store: 상태와 관련 로직, 액션을 받아 상태 업데이트 

View: 스토어의 상태를 받아 화면에 표시, 리액트 컴포넌트 

### 왜 state를 직접 바꾸지 않고 setState를 사용?, setState가 비동기인 이유

직접 값을 바꾸면 리랜더링 트리거 발생x, 객체로 저장된 state를 앝은 비교를 하기 때문 

비동기인 이유는 바뀌는 값을 모아서 한 번에 업데이트해 랜더링 최적화  
